...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO user_logins (email, ip_address, latitude, longitude, city, region, country, user_agent, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    if ($stmt === false) {
        echo "error: " . $conn->error;  // Output the error message if preparation failed
        exit();
    }
    $stmt->bind_param("sssssssss", $email, $ip, $latitude, $longitude, $city, $region, $country, $user_agent, $created_at);
